...
/** Public 0xLeverage API — same default as .env.example; override for staging/self-host. */
const DEFAULT_OXL_API_BASE_URL = "https://levapi.0xleverage.io";

// These are called lazily (not at module load) so tests can override process.env
function getApiBaseUrl(): string {
  const v = process.env.OXL_API_BASE_URL?.trim();
  return v && v.length > 0 ? v : DEFAULT_OXL_API_BASE_URL;
}

let loggedMissingOxlApiKey = false;

function getApiKey(): string {
  const key = process.env.OXL_API_KEY?.trim();
  if (key) return key;
  if (process.env.NODE_ENV === "production") {
    return requireEnv("OXL_API_KEY");
  }
  if (!loggedMissingOxlApiKey) {
    loggedMissingOxlApiKey = true;
    logger.warn({
      event: "oxl_api_key_missing",
      hint: "Set OXL_API_KEY in .env. The dev server can start; connect/trade need a real dapp key from 0xLeverage.",
    });
  }
  throw new TRPCError({
    code: "PRECONDITION_FAILED",
    message:
      "OXL_API_KEY is not set. Add it to your .env file. Request a dapp key from 0xLeverage.",
  });
}
